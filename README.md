...
- CI runs on every pull request and on pushes to both branches as a single Windows job: format, lint, and typecheck first, then the unit and integration tests, Electron build, Playwright end-to-end suite, and a portable exe. The exe is uploaded as a workflow artifact named `opossum-portable-<version>.<build>-<sha>`.
