...
- CI runs on every pull request and on pushes to both branches: format, lint, and typecheck on Linux, then the unit and integration tests, Electron build, Playwright end-to-end suite, and a portable exe on Windows. The exe is uploaded as a workflow artifact named `opossum-portable-<version>.<build>-<sha>`.
